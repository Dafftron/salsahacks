...
const SequenceBuilder = ({ 
  ...
  const handleAutoGenerateTags = useCallback(() => {
    if (sequence.length > 0) {
      ...
      // Solo actualizar si hay tags nuevos que añadir
  ...
  const handleSaveSequence = async () => {
    ...
    try {
      ...
      // Generar tags automáticamente desde los videos de la secuencia
      const autoGeneratedTags = generateSequenceTagsFromVideos()
      
      // Combinar tags automáticos con tags manuales (si los hay)
      const finalTags = { ...autoGeneratedTags }
      
      // Si hay tags manuales, añadirlos o sobrescribir los automáticos
      Object.keys(sequenceTags).forEach(categoryKey => {
        const manualTags = sequenceTags[categoryKey]
        if (Array.isArray(manualTags) && manualTags.length > 0) {
          // Combinar tags automáticos con manuales, evitando duplicados
          const existingTags = finalTags[categoryKey] || []
          ...
          finalTags[categoryKey] = combinedTags
        }
      })
      
      console.log('🏷️ Tags finales de la secuencia:', finalTags)
      ...
      const sequenceData = {
        ...
        videos: sequence,
        style: style,
        tags: finalTags, // Usar tags combinados (automáticos + manuales)
        ...
        <div className="p-4">
          ...
          <div className="mb-6">
            <div className="bg-white rounded-lg shadow-md p-4 border border-gray-200">
              ...
              {/* Botón para generar tags automáticamente */}
              <div className="mb-3">
                <button
                  onClick={() => {
                    const autoTags = generateSequenceTagsFromVideos()
                    setSequenceTags(autoTags)
                    addToast(`🏷️ Tags generados automáticamente desde ${sequence.length} videos`)
                  }}
                  ...
                  title={sequence.length === 0 ? "Añade videos a la secuencia primero" : "Generar tags automáticamente desde los videos"}
                >
                  <svg className="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                    <path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M4 4v5h.582m15.356 2A8.001 8.001 0 004.582 9m0 0H9m11 11v-5h-.581m0 0a8.003 8.003 0 01-15.357-2m15.357 2H15" />
                  </svg>
                  <span>Generar Tags Automáticamente</span>
                  {sequence.length > 0 && (
                    <span className="bg-white bg-opacity-20 px-2 py-0.5 rounded-full text-xs">
                      {sequence.length} video{sequence.length !== 1 ? 's' : ''}
                    </span>
                  )}
                </button>
              </div>
