...
import { Maximize2, Minimize2, Upload, Search, Filter, ChevronDown, ChevronUp, Star, Heart, X, EyeOff } from 'lucide-react'
...
import { subscribeToVideosByStyle, subscribeToPageVideos, updateVideoDocument, toggleVideoLike, checkUserLikedVideo, checkUserFavorite, toggleUserHiddenVideo, checkUserHiddenVideo } from '../services/firebase/firestore'
...
const EventosPage = () => {
  ...
  // Anotar estados por usuario (like/favorite/hidden)
  useEffect(() => {
    if (videos.length > 0 && user) {
      const checkStates = async () => {
        const updated = await Promise.all(videos.map(async video => {
          try {
            const [likeResult, favoriteResult, hiddenResult] = await Promise.all([
              checkUserLikedVideo(video.id, user.uid),
              checkUserFavorite(video.id, user.uid),
              checkUserHiddenVideo(video.id, user.uid),
            ])
            return { ...video, userLiked: likeResult.userLiked, isFavorite: favoriteResult.isFavorite, userHidden: hiddenResult.isHidden }
          } catch (e) {
            return { ...video, userLiked: false, isFavorite: false, userHidden: false }
          }
        }))
        setVideos(updated)
      }
